...
function load(){try{return JSON.parse(localStorage.getItem('aiLotteryLab300'))||{purchases:{loto6:[],loto7:[]},reports:{loto6:[],loto7:[]}}}catch{return {purchases:{loto6:[],loto7:[]},reports:{loto6:[],loto7:[]}}}}
function save(){localStorage.setItem('aiLotteryLab300',JSON.stringify(state))}
function cfg(){return CFG[game]}
function nums(v){return (String(v).match(/\d+/g)||[]).map(Number)}
function validSet(a){return a.length===cfg().pick&&new Set(a).size===a.length&&a.every(n=>n>=1&&n<=cfg().max)}
...
function renderPredictions(){const sets=predictions[game];$('#predictions').innerHTML=sets.map((a,i)=>`<div class="set-card"><b>第${i+1}口</b><div class="balls">${a.map(n=>`<span class="ball">${String(n).padStart(2,'0')}</span>`).join('')}</div><small>合計 ${a.reduce((x,y)=>x+y,0)}｜偶奇 ${a.filter(n=>n%2).length}:${a.filter(n=>n%2===0).length}｜AC ${ac(a)}</small></div>`).join('')||'<p class="muted">まだ生成されていません。</p>';if(sets.length)$('#purchaseSets').value=sets.map(a=>a.join(',')).join('\n')}
function savePurchase(){const drawNo=Number($('#purchaseNo').value);const sets=$('#purchaseSets').value.split(/\n+/).map(nums).filter(a=>a.length);if(!drawNo)return alert('対象回を入力してください');if(sets.length!==5||sets.some(a=>!validSet(a)))return alert(`5口すべてを正しく入力してください（各口${cfg().pick}数字）`);state.purchases[game].unshift({id:Date.now(),drawNo,sets,date:new Date().toISOString()});save();renderPurchases();alert('保存しました')}
function renderPurchases(){const list=state.purchases[game];$('#purchaseHistory').innerHTML=list.map(x=>`<div class="purchase-card"><b>第${x.drawNo}回</b><p>${x.sets.map(a=>a.map(n=>String(n).padStart(2,'0')).join('・')).join('<br>')}</p></div>`).join('')||'<p class="muted">保存履歴なし</p>'}
...
function init(){restoreLatest();$$('[data-game]').forEach(b=>b.onclick=()=>setGame(b.dataset.game));$$('[data-tab]').forEach(b=>b.onclick=()=>setTab(b.dataset.tab));$('#refreshOfficial').onclick=refreshOfficial;$('#generate').onclick=generate;$('#savePurchase').onclick=savePurchase;$('#runVerify').onclick=runVerify;renderAll();setTimeout(refreshOfficial,600)}init();
